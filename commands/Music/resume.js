    ...
    run: async (client, { channel, guild, member }, args) => {
        ...
        if ((member.hasPermission("MANAGE_CHANNELS") || guild.me.voiceChannel.members.filter(m => !m.user.bot).size === 1) && guild.me.voiceChannel.members.find(m=>m.id === member.id)) {
            ...
            if (client.dispatchers.get(guild.id)){
                client.dispatchers.get(guild.id).resume();
                channel.send('The player has been resumed.');
            }


        }else return channel.send('You do not have permission to resume. Make sure you have MANAGE__CHANNELS or is the only person in the vc (You have to be in the vc for this command to work).');
